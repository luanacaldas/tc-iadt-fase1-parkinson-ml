...
Original file is located at
    https://colab.research.google.com/drive/1UWMIe3j_0MwNVsup-epe7WGo9Mx4gDt2
...
for i, feature in enumerate(top_features, 1):
    ...
    plt.title(f'Distribuição {feature} por Classe')
plt.tight_layout()
plt.show()
...
plt.subplot(1, 2, 1)
sns.histplot(data=df, x='PPE', hue='status', kde=True, palette='tab10', multiple="stack")
plt.title('Distribuição de PPE por Classe')

plt.subplot(1, 2, 2)
sns.histplot(data=df, x='spread1', hue='status', kde=True, palette='tab10', multiple="stack")
...
sns.barplot(data=coeficientes.head(10), x='coef', y='feature', hue='feature', palette='coolwarm', legend=False)
...
print(f"    → Consequência: Reavaliação clínica e exames complementares para descartar falso positivo")
...
"""## 9.7. Demonstração do Modelo Preditivo

Para demonstrar o modelo XGBoost treinado, vamos criar uma função que simule a entrada de dados de um novo indivíduo e utilize o modelo para fazer uma predição.

**Passos:**

1. Gerar um ponto de dados simulado (com valores dentro de uma faixa realista).
2. Pré-processar este novo ponto de dados usando o `scaler` que foi ajustado nos dados de treino.
3. Utilizar o `modelo_xgb` treinado para prever a probabilidade e a classe.
4. Apresentar o resultado da predição.
"""

def demonstrar_predicao(modelo, scaler, feature_names):
    """
    Args:
        modelo: modelo treinado (ex: modelo_xgb).
        scaler: StandardScaler ajustado nos dados de treino.
        feature_names: Lista com os nomes das features.

    Returns:
        None. Imprime o resultado da predição.
    """
    print("\n" + "="*70)
    print("DEMONSTRAÇÃO: PREDIÇÃO PARA NOVO INDIVÍDUO ESTUDANTE DA FIAP, CURSO IA PARA DEVS")
    print("="*70)

    # Passo 1: Gerar um ponto de dados simulado
    # Para esta demonstração, criaremos um ponto de dados aleatório
    # com base nas estatísticas descritivas dos dados originais
    # Em um cenário real, obteríamos esses dados de uma nova gravação de voz confiável.

    print("\nSimulando dados de voz para um novo indivíduo estudante da FIAP...")
    # Usar a média e o desvio padrão dos dados originais para gerar valores realistas
    # Excluímos a coluna 'status' que foi a variável alvo
    df_description = df.drop('status', axis=1).describe()
    estudante_fiap_data = {}
    for col in feature_names:
        # Gerar um valor aleatório próximo à média, com variação baseada no desvio padrão
        mean = df_description.loc['mean', col]
        std = df_description.loc['std', col]
        # Limitar a geração para valores razoáveis (ex: média +/- 2*std)
        estudante_fiap_data[col] = [np.random.normal(mean, std)]
        # Opcional: Adicionar limites para evitar valores extremos irreais
        # estudante_fiap_data[col][0] = max(df_description.loc['min', col], estudante_fiap_data[col][0])
        # estudante_fiap_data[col][0] = min(df_description.loc['max', col], estudante_fiap_data[col][0])


    estudante_fiap_df = pd.DataFrame(estudante_fiap_data)
    print("Dados simulados (primeiras 5 features):")
    display(estudante_fiap_df.head(1).T.head(5)) # Exibe transposto para melhor visualização

    # Passo 2: Pré-processar os dados simulados (escalar)
    print("\nPré-processando dados (escalando)...")
    estudante_fiap_scaled = scaler.transform(estudante_fiap_df)
    print("Dados escalados.")

    # Passo 3: Fazer a predição
    print("\nFazendo predição com o modelo XGBoost...")
    # Prever a probabilidade da classe positiva (Parkinson)
    prob_parkinson = modelo.predict_proba(estudante_fiap_scaled)[:, 1][0]
    # Prever a classe (0 ou 1)
    predicao_classe = modelo.predict(estudante_fiap_scaled)[0]

    # Passo 4: Apresentar o resultado
    print("\n=== RESULTADO DA PREDIÇÃO ===")
    print(f"Probabilidade de Parkinson: {prob_parkinson:.4f}")

    if predicao_classe == 1:
        print("Diagnóstico Predito: Parkinson")
        if prob_parkinson > 0.75: # Limiar de alta confiança
            print("  → Alta confiança na predição.")
        elif prob_parkinson > 0.6: # Limiar de confiança moderada
             print("  → Confiança moderada na predição. Pode requerer avaliação adicional.")
        else: # Zona de incerteza para classe 1
             print("  → Baixa confiança na predição. Na zona de incerteza (0.4-0.6). Requer avaliação clínica.")
    else:
        print("Diagnóstico Predito: Saudável")
        if prob_parkinson < 0.25: # Limiar de alta confiança
            print("  → Alta confiança na predição.")
        elif prob_parkinson < 0.4: # Limiar de confiança moderada
             print("  → Confiança moderada na predição. Pode requerer avaliação adicional.")
        else: # Zona de incerteza para classe 0
             print("  → Baixa confiança na predição. Na zona de incerteza (0.4-0.6). Requer avaliação clínica.")

    print("="*70)

# Executar a demonstração
# Usamos o modelo_xgb treinado, o scaler ajustado e os nomes das colunas originais
demonstrar_predicao(modelo_xgb, scaler, X.columns)

"""### 9.8 Conclusão da Análise de Aprendizado
...
Código-fonte completo disponível em: https://github.com/luanacaldas/tc-iadt-fase1-parkinson-ml
